...
class DmsPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `dms`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM dms WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO dms (date, title, link, content)"
